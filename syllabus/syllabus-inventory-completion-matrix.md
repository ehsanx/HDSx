...
| Week 6 | Data Visualization | `weeks/week06-visualization/index.qmd`; `viz01`-`viz07` incremental pages; A5; M1 visual plan | Ready | Seven-page sequence (overview, grammar+first plot, worked example, misleading-design audit, AI prompts, studio, A5 walkthrough+reference). Instructor key files unlinked from student-facing pages; see `instructor/MAINTENANCE.md`. |
| Week 7 | EDA, Table 1 & AI Auditing | `weeks/week07-eda-ai-audit/index.qmd`; `eda01`-`eda07` incremental pages; A6; M2 | Ready | Seven-page sequence (overview, cohort+missingness, worked example, descriptive-statistic claims, four audit categories, planted-error studio, A6 walkthrough+reference). Planted-error key unlinked from student-facing pages; see `instructor/MAINTENANCE.md`. |
