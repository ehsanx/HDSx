...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, arc=.35mm, left=2mm, coltitle=black, toprule=.15mm, bottomtitle=1mm, colframe=quarto-callout-note-color-frame, colback=white, opacityback=0, opacitybacktitle=0.6, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, titlerule=0mm, breakable, toptitle=1mm, rightrule=.15mm, leftrule=.75mm, colbacktitle=quarto-callout-note-color!10!white]
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, arc=.35mm, left=2mm, coltitle=black, toprule=.15mm, bottomtitle=1mm, colframe=quarto-callout-warning-color-frame, colback=white, opacityback=0, opacitybacktitle=0.6, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Content TODO}, titlerule=0mm, breakable, toptitle=1mm, rightrule=.15mm, leftrule=.75mm, colbacktitle=quarto-callout-warning-color!10!white]
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, arc=.35mm, left=2mm, coltitle=black, toprule=.15mm, bottomtitle=1mm, colframe=quarto-callout-tip-color-frame, colback=white, opacityback=0, opacitybacktitle=0.6, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip}, titlerule=0mm, breakable, toptitle=1mm, rightrule=.15mm, leftrule=.75mm, colbacktitle=quarto-callout-tip-color!10!white]
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, arc=.35mm, left=2mm, coltitle=black, toprule=.15mm, bottomtitle=1mm, colframe=quarto-callout-warning-color-frame, colback=white, opacityback=0, opacitybacktitle=0.6, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Warning}, titlerule=0mm, breakable, toptitle=1mm, rightrule=.15mm, leftrule=.75mm, colbacktitle=quarto-callout-warning-color!10!white]
...
\section*{Tutorial Map}\label{tutorial-map}
\addcontentsline{toc}{section}{Tutorial Map}

\markright{Tutorial Map}
...
\tightlist
\item
  \href{python01-when-and-why.qmd}{Python vs.~R - When and Why?}
\item
  \href{python02-environment.qmd}{Setting Up Your Python Environment}
\item
  \href{python03-packages-reproducibility.qmd}{Packages and
  Reproducibility}
\item
  \href{python04-translation-traps.qmd}{Common Translation Traps}
\item
  \href{python05-load-inspect-data.qmd}{Loading and Inspecting Health
  Data}
\item
  \href{python06-tracebacks.qmd}{Reading Python Tracebacks}
\item
  \href{python07-one-step-one-cell.qmd}{The One Step, One Cell AI
  Workflow}
\item
  \href{python08-mixing-r-python.qmd}{Mixing R and Python}
\item
  \href{python09-reusable-scripts.qmd}{Creating Reusable Python Scripts}
\item
  \href{python10-git-progress.qmd}{Tracking Your Progress With Git}
\item
  \href{python11-project-milestone.qmd}{Project Proposal Readiness
  Check}
\item
  \href{python12-knowledge-check.qmd}{Knowledge Check}
\item
  \href{python13-translation-exercise.qmd}{The Bilingual Translation
  Exercise}
\item
  \href{python14-cheat-sheet.qmd}{R To Python Cheat Sheet}
\end{enumerate}
...
\chapter{Tutorial 6: Reading Python
Tracebacks}\label{tutorial-6-reading-python-tracebacks}
...
\chapter{Tutorial 11: Project Proposal Readiness
Check}\label{tutorial-11-project-proposal-readiness-check}
...
\chapter{Tutorial 12: Knowledge
Check}\label{tutorial-12-knowledge-check}
...
\chapter*{NHANES Equity Case Study Teaching
Notes}\label{nhanes-equity-case-study-teaching-notes}
\addcontentsline{toc}{chapter}{NHANES Equity Case Study Teaching Notes}

\markboth{NHANES Equity Case Study Teaching Notes}{NHANES Equity Case
...
This note gives RAs and instructors the teaching plan for the recurring
NHANES Health Equity Dashboard case study. The case study should
function as a course spine: students revisit the same app, data, and
workflow from different angles as their skills mature. It should not
become a second term project.

\section*{What Was Implemented}\label{what-was-implemented}
\addcontentsline{toc}{section}{What Was Implemented}

\markright{What Was Implemented}

The case study now lives in one self-contained folder:
...
The app was adapted from \texttt{examples/bmi39.R}. The original task
brief referred to \texttt{examples/bmi.39.R}, but that file was not
present in this checkout.

The cached data are stored in \texttt{examples/nhanes-equity/data/} so
the case-study app, script, README, and weekly snippets all use one
consistent location. Legacy copies under \texttt{examples/data/} were
left untouched to avoid breaking older references.

The weekly index pages for Weeks 1-11 now include phased case-study
sections. Weeks 1-7 use data-first headings and snippets; Weeks 8-11 use
the dashboard-forward snippet once app work becomes central.

\section*{Pedagogical Design}\label{pedagogical-design}
\addcontentsline{toc}{section}{Pedagogical Design}

\markright{Pedagogical Design}

The strongest use of this case study is incremental. Students should
first learn to inspect and document a provided artifact, then audit and
compare code, then critique visual and KT choices, and only later
perform a fresh-run reproducibility stress test.
...
The cached RDS is the default classroom path. Rebuilding from CDC
through \texttt{nhanesA} is valuable, but it is network-dependent and
can be slow. Treat CDC rebuilds as an instructor demo or Week 11
challenge unless the course explicitly allocates time for them.

\section*{Week-by-Week Instructor
Notes}\label{week-by-week-instructor-notes}
\addcontentsline{toc}{section}{Week-by-Week Instructor Notes}

\markright{Week-by-Week Instructor Notes}

Week 1 introduces the case study through the Data Intake Card. Keep the
focus on provenance, stewardship, and KT framing. Students do not need
to run the app.

Week 2 uses the case study to make repo structure concrete. Students
should locate the app, data, script, and snippet; confirm that paths are
relative; and render the book.

Week 3 uses the CSV snapshot as a code-reading and data-inspection
object. Students load, inspect, summarize, and ask AI to comment on
code, then audit the comments against the actual operations.

Week 4 uses the case study for Git practice. The safest PR-sized change
is documentation-only: label, caption, README wording, or a limitation
note. Avoid asking students to add new analytic variables during the Git
week.

Week 5 uses the CSV snapshot for R/Python parity. The goal is not Python
mastery; it is learning how plausible outputs can diverge because of
missing values, data types, or grouping choices.

Week 6 uses the worked ggplot example, flawed instructor plot, and
visual-audit activity. Prefer instructor-provided flawed examples over
asking students to invent misleading visuals from scratch.

Week 7 uses the worked Table 1 example and planted-error starter. Survey
weights, strata, and PSU columns can be identified, but design-based
standard errors should be instructor/demo material unless already
taught.

Week 8 is the first app-forward week. Students should run the app,
identify an audience, and improve one audience-facing label, help text,
caption, or interpretation note. Deployment should remain optional
unless explicitly assigned.

Week 9 uses one dashboard plot or table as the basis for a short slide
explanation. Students should include a plain-language takeaway, a
limitation, and a source note.

Week 10 turns the dashboard into a companion report. The report should
document data, methods, limitations, and source notes rather than
duplicating the dashboard interface.

Week 11 stress-tests reproducibility. Students should launch the app
from a fresh session, confirm the cached RDS path, and optionally
rebuild from CDC if internet access and time allow.

\section*{What RAs Should Watch For}\label{what-ras-should-watch-for}
\addcontentsline{toc}{section}{What RAs Should Watch For}

\markright{What RAs Should Watch For}
...
\section*{TODO Decisions}\label{todo-decisions}
\addcontentsline{toc}{section}{TODO Decisions}

\markright{TODO Decisions}

\begin{itemize}
\tightlist
\item
  TODO: Decide whether the legacy files in \texttt{examples/data/}
  should stay for backward compatibility or be retired after all
  references are updated.
\item
  TODO: Decide whether \texttt{examples/bmi39.R} remains as a historical
  source file or becomes a short redirect note to
  \texttt{examples/nhanes-equity/app/app.R}.
\item
  TODO: Decide whether CDC-rebuilt data should be committed each term or
  treated as generated output.
\item
  TODO: Decide whether Week 5's R/Python parity check is optional
  practice or part of an assignment.
\item
  TODO: Decide whether Week 8 includes only local app use or also
  demonstrates deployment.
\item
  TODO: Decide whether the Week 6 flawed plot is distributed as code,
  image, or both.
\item
  TODO: Decide whether the Week 7 planted-error starter is distributed
  from the instructor folder or copied into student assignment repos.
\item
  TODO: Choose any additional required Table 1 variables for Week 7
  beyond N, BMI mean (SD), and age mean (SD).
\end{itemize}

\section*{Definition of Done for This Case
Study}\label{definition-of-done-for-this-case-study}
\addcontentsline{toc}{section}{Definition of Done for This Case Study}

\markright{Definition of Done for This Case Study}
