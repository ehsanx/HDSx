...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, rightrule=.15mm, colback=white, opacitybacktitle=0.6, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, toptitle=1mm, colframe=quarto-callout-note-color-frame, leftrule=.75mm, breakable, toprule=.15mm, bottomtitle=1mm, left=2mm, bottomrule=.15mm, arc=.35mm, titlerule=0mm, coltitle=black, colbacktitle=quarto-callout-note-color!10!white]
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, rightrule=.15mm, colback=white, opacitybacktitle=0.6, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Content TODO}, toptitle=1mm, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, breakable, toprule=.15mm, bottomtitle=1mm, left=2mm, bottomrule=.15mm, arc=.35mm, titlerule=0mm, coltitle=black, colbacktitle=quarto-callout-warning-color!10!white]
...
\section*{Approved Starter Portals (choose
one)}\label{approved-starter-portals-choose-one}
\addcontentsline{toc}{section}{Approved Starter Portals (choose one)}

\markright{Approved Starter Portals (choose one)}

Choose \textbf{one} dataset/indicator page from one of the portals below
(pick something you care about). You are \textbf{not} required to use
the same dataset as other students.

\begin{enumerate}
\def\labelenumi{\arabic{enumi})}
\tightlist
\item
  \textbf{Survey-style microdata (person-level):} NHANES or NHIS (see
  Reference: Data Sources)

  \begin{itemize}
  \tightlist
  \item
    Good if you want person-level variables (age, sex, BMI, smoking,
    etc.)
  \item
    Expect: codebooks, sampling notes, and data dictionaries
  \end{itemize}
\item
  \textbf{Indicators (aggregated country/province-level): WHO Global
  Health Observatory (GHO)}

  \begin{itemize}
  \tightlist
  \item
    Good if you want country-level trends (life expectancy, mortality,
    vaccination coverage, etc.)
  \item
    Expect: ``indicator metadata'' (definition + method), downloadable
    CSV, \textbf{not} person-level records
  \end{itemize}

  \textbf{How to use GHO quickly (what to click):}

  \begin{itemize}
  \tightlist
  \item
    Pick \textbf{one indicator} (e.g., ``Life expectancy at birth'',
    ``Maternal mortality ratio'', ``Diabetes prevalence'')
  \item
    Pick \textbf{one geography} (Canada or another country/region)
  \item
    Find: indicator definition / metadata, time coverage, and the
    download option
  \end{itemize}
\item
  \textbf{Canadian open data portal:} BC Data Catalogue or Open
  ...
  \item
    Expect: variable definitions vary widely (sometimes excellent,
    sometimes minimal)
  \end{itemize}
\end{enumerate}

\begin{quote}
Tip: If you're unsure whether your selection is microdata or aggregated
indicators, write ``unknown'' in \texttt{data\_level} and explain what
you observed on the download page.
\end{quote}
...
\section*{Example (Completed Data Intake Card ---
sample)}\label{example-completed-data-intake-card-sample}
\addcontentsline{toc}{section}{Example (Completed Data Intake Card ---
sample)}

\markright{Example (Completed Data Intake Card --- sample)}

\subsection*{WHO}\label{who}
\addcontentsline{toc}{subsection}{WHO}

This example uses \textbf{one WHO indicator} to demonstrate what
``Complete'' looks like. You may choose a different portal and topic.

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\FunctionTok{dataset\_name}\KeywordTok{:}\AttributeTok{ }\StringTok{"Life expectancy at birth (years)"}
\FunctionTok{publisher\_org}\KeywordTok{:}\AttributeTok{ }\StringTok{"World Health Organization (WHO)"}
\FunctionTok{permalink\_url}\KeywordTok{:}\AttributeTok{ }\StringTok{"https://www.who.int/data/gho"}
\FunctionTok{license\_terms}\KeywordTok{:}\AttributeTok{ }\StringTok{"See WHO terms on the portal; record as provided"}
\FunctionTok{geography}\KeywordTok{:}\AttributeTok{ }\StringTok{"Canada (and global comparison)"}
\FunctionTok{time\_coverage}\KeywordTok{:}\AttributeTok{ }\StringTok{"2000–2023 (varies by country)"}
\FunctionTok{data\_level}\KeywordTok{:}\AttributeTok{ }\StringTok{"Aggregated indicators (country{-}level)"}
\FunctionTok{variables\_of\_interest}\KeywordTok{:}
\AttributeTok{  }\KeywordTok{{-}}\AttributeTok{ }\FunctionTok{name}\KeywordTok{:}\AttributeTok{ }\StringTok{"Life expectancy at birth (years)"}
\AttributeTok{    }\FunctionTok{definition}\KeywordTok{:}\AttributeTok{ }\StringTok{"Expected years of life at birth (method described in WHO metadata)"}
\AttributeTok{    }\FunctionTok{known\_limitations}\KeywordTok{:}\AttributeTok{ }\StringTok{"Model{-}based estimates; depends on reporting quality; not person{-}level"}
\FunctionTok{stewardship\_notes}\KeywordTok{:}\AttributeTok{ }\StringTok{"No individual{-}level data; still avoid overclaiming causality; cite metadata."}
\FunctionTok{ai\_use\_note}\KeywordTok{:}\AttributeTok{ }\StringTok{"Used AI to draft a plain{-}language summary of the indicator; verified against WHO metadata page."}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\FunctionTok{dataset\_name}\KeywordTok{:}\AttributeTok{ }\StringTok{"NHANES 2017–2018 Demographics (example)"}
\FunctionTok{publisher\_org}\KeywordTok{:}\AttributeTok{ }\StringTok{"CDC / NCHS"}
\FunctionTok{permalink\_url}\KeywordTok{:}\AttributeTok{ }\StringTok{"https://wwwn.cdc.gov/nchs/nhanes/"}
\FunctionTok{date\_accessed}\KeywordTok{:}\AttributeTok{ }\StringTok{"YYYY{-}MM{-}DD"}
\FunctionTok{license\_terms}\KeywordTok{:}\AttributeTok{ }\StringTok{"Public{-}use (see NHANES guidance on site)"}
\FunctionTok{geography}\KeywordTok{:}\AttributeTok{ }\StringTok{"United States"}
\FunctionTok{time\_coverage}\KeywordTok{:}\AttributeTok{ }\StringTok{"2017–2018"}
\FunctionTok{data\_level}\KeywordTok{:}\AttributeTok{ }\StringTok{"Microdata (person{-}level)"}
\FunctionTok{variables\_of\_interest}\KeywordTok{:}
\AttributeTok{  }\KeywordTok{{-}}\AttributeTok{ }\FunctionTok{name}\KeywordTok{:}\AttributeTok{ }\StringTok{"RIAGENDR"}
\AttributeTok{    }\FunctionTok{definition}\KeywordTok{:}\AttributeTok{ }\StringTok{"Gender (coded categories per documentation)"}
\FunctionTok{known\_limitations}\KeywordTok{:}
\AttributeTok{  }\KeywordTok{{-}}\AttributeTok{ }\StringTok{"Complex survey design; weights needed for population inference"}
\FunctionTok{stewardship\_notes}\KeywordTok{:}\AttributeTok{ }\StringTok{"De{-}identified public{-}use microdata; still avoid reporting small cell counts."}
\FunctionTok{ai\_use\_note}\KeywordTok{:}\AttributeTok{ }\StringTok{"Used AI to summarize the codebook section for RIAGENDR; verified against official documentation."}
\end{Highlighting}
\end{Shaded}

\section*{Completion Checklist (Complete /
Incomplete)}\label{completion-checklist-complete-incomplete}
\addcontentsline{toc}{section}{Completion Checklist (Complete /
Incomplete)}

\markright{Completion Checklist (Complete / Incomplete)}
...
\item[$\square$]
  At least \textbf{one} variable with a definition grounded in
  documentation (not AI guesswork)
\item[$\square$]
  One limitation or risk of misuse (privacy, over-interpretation,
  stewardship)
\item[$\square$]
...
\begin{quote}
If you're uncertain about something, write \textbf{unknown} and explain
what you looked for. \#\# TODO Before Release
\end{quote}

\section*{Submission (Week 1)}\label{submission-week-1}
\addcontentsline{toc}{section}{Submission (Week 1)}

\markright{Submission (Week 1)}

This is \textbf{not graded}, but you must complete it because you'll
reuse it later.

Submit by: - \textbf{Option A (recommended):} paste your completed card
into Canvas (Week 1 module) - \textbf{Option B:} commit a
\texttt{data-intake-card.md} file to your repo (we'll do this starting
Week 2)
...
Mini-tasks for Week 2:

\begin{itemize}
\tightlist
\item
  Locate the app, data, script, and snippet folders in the Explorer.
\item
  Explain why the app uses relative paths instead of machine-specific
  paths.
\item
  Confirm that the weekly page includes the shared snippet rather than
  duplicating instructions.
\item
  Render the book from the repository root and confirm this page updates
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, rightrule=.15mm, colback=white, opacitybacktitle=0.6, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip}, toptitle=1mm, colframe=quarto-callout-tip-color-frame, leftrule=.75mm, breakable, toprule=.15mm, bottomtitle=1mm, left=2mm, bottomrule=.15mm, arc=.35mm, titlerule=0mm, coltitle=black, colbacktitle=quarto-callout-tip-color!10!white]
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, rightrule=.15mm, colback=white, opacitybacktitle=0.6, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Warning}, toptitle=1mm, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, breakable, toprule=.15mm, bottomtitle=1mm, left=2mm, bottomrule=.15mm, arc=.35mm, titlerule=0mm, coltitle=black, colbacktitle=quarto-callout-warning-color!10!white]
...
for routine class work; any CDC retrieval pipeline is optional or
instructor-directed.
...
\item
  ...
  logic.
\item
  Open a pull request and use the diff to confirm the change is limited.
\item
...
\section*{Instructor Decisions}\label{instructor-decisions}
\addcontentsline{toc}{section}{Instructor Decisions}

\markright{Instructor Decisions}

\begin{tcolorbox}[enhanced jigsaw, opacityback=0, rightrule=.15mm, colback=white, opacitybacktitle=0.6, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Before Release}, toptitle=1mm, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, breakable, toprule=.15mm, bottomtitle=1mm, left=2mm, bottomrule=.15mm, arc=.35mm, titlerule=0mm, coltitle=black, colbacktitle=quarto-callout-warning-color!10!white]

\begin{itemize}
\tightlist
\item
  Confirm whether the Week 6 worked example becomes required pre-class
  work or in-class practice.
\item
  Decide whether the instructor-provided flawed chart is released as
  code, image, or both.
\item
  Decide whether a plotly or interactive chart extension is required or
  optional.
\item
...
\section*{Instructor Decisions}\label{instructor-decisions-1}
\addcontentsline{toc}{section}{Instructor Decisions}
...
\tightlist
\item
  Confirm whether the Table 1 worked example becomes required pre-class
  work or in-class practice.
\item
  Decide whether the planted-error starter is distributed as-is or
  copied into student assignment repos.
\item
...
\item
  Treat deployment as optional unless the instructor explicitly assigns
  it.
\end{itemize}
...
\item
  ...
  pathway.
\item
  Confirm that the report uses relative paths and documents whether
...
\subsection*{Optional Extension: Static Dashboard (No
Server)}\label{optional-extension-static-dashboard-no-server}
\addcontentsline{toc}{subsection}{Optional Extension: Static Dashboard
(No Server)}

Advanced students can compare the Shiny workflow with the draft
\href{static-dashboard-ojs.qmd}{static dashboard source file}. This
extension reads from a CSV snapshot and does not require a Shiny server;
render it as a standalone dashboard if assigned.
...
revise it, and explain why your version is more accurate and useful for
...
\tightlist
\item
  Review the Week 6 worked example and note how it labels data source,
  statistic, and limitations.
\item
  Choose one comparison from the NHANES Health Equity CSV snapshot.
\item
  Draft an initial plot using R and \texttt{ggplot2}.
\item
  Audit the plot for misleading scale choices, unclear labels, missing
  context, and overclaiming.
\item
...
\section{Submission Location}\label{submission-location}
...
\item
  Revised plot is not materially different from the flawed or draft
  version.
\end{itemize}
...
\item
  Define and document an analysis cohort, using age 20-80 unless
...
\item
  Include at minimum N, BMI mean (SD), and age mean (SD).
\item
  Write a short methods note that states the analysis is descriptive,
...
\section{Submission Location}\label{submission-location-1}
