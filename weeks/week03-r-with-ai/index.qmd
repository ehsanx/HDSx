...
<!-- AI-EDIT(2026-06-11): TF-013, MIT-030, WI-017 — needs review -->
...
Before you start, make sure your tools are ready: [The Setup (The Cockpit)](r01-setup.qmd). <!-- AI-EDIT(2026-06-11): GAP-R345-1 — needs review -->
<!-- AI-EDIT(2026-06-22): REVIEW-row55, GAP-R345-1 — RA requests an added screenshot on the linked Setup page (r01-setup.qmd); image not available to insert. NEEDS-RA-SCREENSHOT -->
<!-- TODO: RA to provide screenshot for r01-setup.qmd (referenced by REVIEW-row55) -->

<!-- AI-EDIT(2026-06-11): TF-013, MIT-025, WI-018 — needs review -->
::: {.callout-note title="How to use the code blocks on this page"}
...
**What to do with this code (the case-study block below):** Run — it loads the cached NHANES CSV and prints two summaries. You do not need to modify it this week. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
<!-- AI-EDIT(2026-06-11): MIT-033, GAP-R345-1 — needs review -->
## Packages: Opening the Toolbox

Deeper dive: [Packages (The Toolbox)](r02-packages.qmd).
...
**What to do with this code:** Run — execute it as written every time you reopen your Codespace. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->

<!-- AI-EDIT(2026-06-22): REVIEW-row59, MIT-025 — clarify you must be inside an R session (the `>` prompt) before running library() -->
...
**What to do with this code:** Run `renv::status()` as written; Read the other two commands so you recognize them. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->

<!-- AI-EDIT(2026-06-22): REVIEW-row60,row68, MIT-025 — clarify renv commands run inside an R session, not the bash terminal; corrected RA's renv::init() advice (project is pre-initialized, so init is not needed) -->
**Where to run this:** These are R commands, so run them *inside* an R session (the `>` prompt), not at the plain terminal. If you are not already in R, open a terminal (**Terminal → New Terminal**), type `R`, press Enter, then run `renv::status()`. The course project is already set up for `renv`, so you do **not** need to run `renv::init()` yourself.
...
Use `renv::status()` to check whether the project environment matches the lockfile. Do not run `renv::snapshot()` unless the instructor asks or you have intentionally added a package, because it changes the recorded package state.

<!-- AI-EDIT(2026-06-22): REVIEW-row61, GAP-R345-10 — show expected output and a restart fallback for renv::status() -->
When everything is consistent, `renv::status()` prints:

```
No issues found -- the project is in a consistent state.
```

If instead it reports that packages are out of sync, first try restarting R (type `q()` to quit, choose **n** when asked to save the workspace, then type `R` again) and rerun `renv::status()`. If it still reports a mismatch, do **not** run `renv::snapshot()` to "fix" it --- use the Canvas support channel or office hours and include the exact message, since the lockfile is instructor-managed.
...
Deeper dives: [Basics: Boxes & Toasters](r03-basics.qmd); the pipe is also covered in [Control Flow](r04-control-flow.qmd).

### Variables: The Labeled Box

R has no memory unless you store results in a named box using `<-`.

**What to do with this code:** Run — type it in the Console, then check the Environment pane to see your new "boxes". <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
**What to do with this code:** Run — execute it as written. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
**What to do with this code:** Read — `data` here is a placeholder, so this block will not run as-is. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
Deeper dive: [The Loading Dock (Data Import)](r05-data-import.qmd).

### Loading Data

Use relative paths so your code runs on any machine (including your grader's Codespace).

**What to do with this code:** Modify — replace the path with the relative path to your own file, then run it. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
**What to do with this code:** Run — execute it as written once your data is loaded. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->

<!-- AI-EDIT(2026-06-22): REVIEW-row69, MIT-025 — note that glimpse() needs tidyverse loaded and state where to run -->
...
<!-- AI-EDIT(2026-06-11): MIT-034 — needs review -->
...
## Data Transformation: The Five `dplyr` Verbs <!-- AI-EDIT(2026-06-11): MIT-033 — needs review -->
...
**What to do with the code in this section:** Modify — swap in your own dataset and column names, then run each block. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
Deeper dive: [Control Flow](r04-control-flow.qmd).

You will not write many `if/else` blocks or loops by hand. But AI frequently generates them, so you need to **read** them.

**What to do with the code in this section:** Read — these are patterns to recognize in AI output; you do not need to run them. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
## Writing a Simple Function <!-- AI-EDIT(2026-06-11): MIT-033 — needs review -->
...
**What to do with this code:** Run, then Modify — run it as written on your data, then change one thing as described in "Your Task" below. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
Deeper dive: [The Detective (Debugging)](r06-debugging.qmd).
...
<!-- AI-EDIT(2026-06-11): WI-019 — needs review -->
...
Deeper dive: [AI Safety: The Audit Checklist](r07-ai-audit-checklist.qmd).
...
Deeper dive: [The Paper Trail (Comments)](r08-comments.qmd). <!-- AI-EDIT(2026-06-11): GAP-R345-1 — needs review -->

Every time you use AI to write a code chunk, add a `#` comment above it explaining what you asked for and what you verified:

**What to do with this code:** Read — then imitate this comment pattern above your own AI-assisted chunks. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
## Exporting Clean Data <!-- AI-EDIT(2026-06-11): MIT-033 — needs review -->

After cleaning and transforming, save your work so future weeks can pick up where you left off.

**What to do with this code:** Run — it creates `output/clean_data.csv`; create the `output/` folder first if it does not exist. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
## Literate Analysis in Quarto <!-- AI-EDIT(2026-06-11): MIT-033 — needs review -->
...
**What to do with this code:** Read — a pattern to imitate in your own document, not text to paste as-is. <!-- AI-EDIT(2026-06-11): MIT-025 — needs review -->
...
## Knowledge Check <!-- AI-EDIT(2026-06-11): MIT-033 — needs review -->
...
<!-- AI-EDIT(2026-06-11): MIT-032, MIT-033, GAP-R345-1 — needs review -->
## Assignment: The "Mad Libs" Report

Full instructions: [Assignment 2 README](../../assignments/assignment02-r-with-ai/README.md). Short version: [Assignment: The "Mad Libs" Report](r09-assignment-mad-libs.qmd).
...
1. Open the [Assignment 2 README](../../assignments/assignment02-r-with-ai/README.md) and follow its setup steps: copy the starter file (`assignments/assignment02-r-with-ai/starter-analysis.qmd`) into your submission folder as `analysis-note.qmd`. The dataset is `examples/nhanes-equity/data/nhanes_equity_v6.csv` ([view on GitHub](https://github.com/ehsanx/HDSx/blob/main/examples/nhanes-equity/data/nhanes_equity_v6.csv)).
...
See also: [Final Summary Checklist](r10-final-checklist.qmd).
