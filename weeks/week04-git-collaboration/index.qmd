...
As your projects grow, you will generate files that should *not* be tracked:
temporary Quarto support folders, local caches, `.DS_Store`, and API keys.
Rendered files explicitly required by an assessment are different: they are
part of the evidence and must be committed.
...
3.  Stage and commit with a message such as `"Add project gitignore rules"`.

<!-- AI-EDIT(2026-06-11): MIT-169 (D-006) — needs review -->
::: {.callout-note title="Required renders are committed"}
Do **not** add global `*.html` or `*.pdf` patterns to the course workspace
`.gitignore`. If an assignment or milestone explicitly lists a rendered HTML or
PDF file, commit it with the source. For practice renders that are not requested,
use Source Control to stage only the files named in the brief.
:::
