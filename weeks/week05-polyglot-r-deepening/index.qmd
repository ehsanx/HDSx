...
**What to do with this code:** read it. The code block below runs automatically when this page is built — you do not need to run or change it here. You will write your own version in the studio. Direct links to the files it uses: [nhanes_equity_v6.csv (view on GitHub)](https://github.com/ehsanx/HDSx/blob/main/examples/nhanes-equity/data/nhanes_equity_v6.csv), [nhanes_equity_v6.rds (view on GitHub)](https://github.com/ehsanx/HDSx/blob/main/examples/nhanes-equity/data/nhanes_equity_v6.rds), and the [case-study README (view on GitHub)](https://github.com/ehsanx/HDSx/blob/main/examples/nhanes-equity/README.md).
...
You do not edit an existing file for this studio. Create one new R script named `week5.R`, saved inside the Week 5 folder (`weeks/week05-polyglot-r-deepening/`). Check the Explorer panel before saving so the file does not land in the repository root. For Assignment 4, the same code later moves into `polyglot-parity.qmd` inside `assignments/assignment04-polyglot/submission/`.

### R Pipeline and R Function

The studio code — a `dplyr` pipeline and a small reusable function — lives in one place so there is a single copy to keep correct: work through Parts A and B of [R Deepening and Parity Check](r-deepening-parity.qmd). Type or paste each block into `week5.R` and run it.
...
R steps happen in your `week5.R` script. Python steps happen in your `translation.ipynb` notebook (see [Python Environment](python02-environment.qmd) for how to create it and run cells). `pandas` comes preinstalled in the course Codespace — you do not need to install anything.
