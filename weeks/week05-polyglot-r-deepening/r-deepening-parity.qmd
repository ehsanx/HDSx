...
Write Parts A and B in one new R script named `week5.R`, saved in the Week 5 folder (`weeks/week05-polyglot-r-deepening/`). You are not editing an existing file. For Assignment 4, the same code later moves into `polyglot-parity.qmd` in `assignments/assignment04-polyglot/submission/`.
...
The pipeline and function above are the worked example. Complete at least two of these short exercises in `week5.R` before moving to Python:
...
Create the Python translation in a notebook named `translation.ipynb`, saved in the Week 5 folder (`weeks/week05-polyglot-r-deepening/`) — the same notebook you set up in [Python Environment](python02-environment.qmd) and use in [Bilingual Translation](python13-translation-exercise.qmd). Attempt each step yourself first with the [cheat sheet](python14-cheat-sheet.qmd); use AI (the audit prompt above) to check or fix one step at a time, never to generate the full script.
